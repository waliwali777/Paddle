...
class TestNanmedianModeMin(unittest.TestCase):
    ...
    def test_api_static(self):
        ...
        with paddle.static.program_guard(paddle.static.Program()):
            ...
            out1, _ = paddle.nanmedian(x, keepdim=False, mode='min')
            out2, _ = paddle.tensor.nanmedian(x, keepdim=False, mode='min')
            out3, _ = paddle.tensor.stat.nanmedian(x, keepdim=False, mode='min')
    ...
    def test_check_grad(self):
        ...
        y, _ = paddle.nanmedian(x_tensor, keepdim=True, mode='min')
    ...
    def test_mode_min_index(self):
        ...
        out, index = paddle.nanmedian(x, mode='min')
        np.testing.assert_allclose(out.numpy(), 99.0)
        np.testing.assert_equal(index.numpy(), 99)

    def test_check_grad_0d(self):
        paddle.disable_static(place=self.place)
        x = paddle.rand([])
        x.stop_gradient = False
        y, _ = paddle.nanmedian(x, mode='min')
        y.backward()
