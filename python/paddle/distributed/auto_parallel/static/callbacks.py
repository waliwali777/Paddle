...
def config_callbacks(
    ...
    if synchronized_IPS_configs['enabled'] and not any(
        isinstance(k, SynchronizedIPSMetric) for k in cbks
    ):
        cbks = cbks + [
            SynchronizedIPSMetric(
                enabled=synchronized_IPS_configs['enabled'],
                sync_every_n_steps=synchronized_IPS_configs[
                    'sync_every_n_steps'
                ],
                visualize_sync_time=synchronized_IPS_configs[
                    'visualize_sync_time'
                ],
            )
        ]
...
class SynchronizedIPSMetric(Callback):
    ...
    Args:
        ...
        sync_every_n_steps (int): send a synchronize request to device every n iterations.
    ...
    def __init__(
        ...
        if self.enabled:
            sys.stdout.write('** Enabled synchronized IPS measuring\n')
    ...
    def on_train_batch_end(self, step, logs=None):
        ...
        if self._iter_counter <= 0:
            ...
            if len(self.time_stamps) >= 2:
                ...
                message = '** IPS (w/ sync): %.4f samples/sec **' % (
                    num_steps / dt
                )
